...
acc_svd = evaluate.nearsub(Z_train, y_train, Z_test, y_test, len(args.classes), n_comp=100)
...
acc_svd = evaluate.nearsub(Z_train, y_train, Z_translate, y_translate, len(args.classes), n_comp=100)
acc = {"svm": acc_svm, "knn": acc_knn, "nearsub-svd": acc_svd} 
utils.save_params(model_dir, acc, name="acc_translate.json")

# plot
plot.plot_combined_loss(model_dir)
plot.plot_heatmap(X_train, y_train, "X_train", len(args.classes), model_dir)
plot.plot_heatmap(X_test, y_test, "X_test", len(args.classes), model_dir)
plot.plot_heatmap(X_translate, y_translate, "X_translate", len(args.classes), model_dir)
plot.plot_heatmap(Z_train, y_train, "Z_train", len(args.classes), model_dir)
plot.plot_heatmap(Z_test, y_test, "Z_test", len(args.classes), model_dir)
plot.plot_heatmap(Z_translate, y_translate, "Z_translate", len(args.classes), model_dir)
